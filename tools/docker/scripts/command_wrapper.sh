    ...
    case "$arg" in
        ...
        *)
            if [ "$CLI_TARGET_SEEN" -eq 0 ]; then
                ...
                echo "Usage: build [pdf|spellcheck|tex|clean] [template-name] [source-dir]"
                echo "   or: build [--targets=pdf,spellcheck,tex,clean] [--template=da-base-template] [--source-dir=/workspace]"
                exit 1
            fi
            ;;
    esac
